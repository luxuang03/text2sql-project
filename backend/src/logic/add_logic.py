...
# Valida i 7 campi e sincronizza film, regista e piattaforme nel DB
def _handle_add_parts(parts: list[str]) -> dict:
    ...
    try:
        ...
        raise AddLineFormatError("Età regista e anno devono essere interi.")
...
# Divide la riga CSV in 7 campi e li passa alla funzione principale
def handle_add(line: str) -> dict:
    parts = [p.strip() for p in line.strip().split(",")]
    return _handle_add_parts(parts)


# Add usata per il popolamento dal file data.tsv
